layui.use(['jquery', 'laydate', 'util', 'layer', 'table', 'form', 'layedit'], function ($, laydate, util, layer, table,
            ...
            $('#formSubmit').click(function event() {
                form.on('submit(formSubmit)', function (data) {
                    //                    console.log('form:' + JSON.stringify(data.field));
                    ...
                    var jsonData = JSON.parse(JSON.stringify(data.field));

                    if(jsonData.methodType=='Get'){
                        methodType = 'Get'
                        ContentType = 'application/x-www-form-urlencoded';
                    }else if(jsonData.methodType=='Post'){
                        methodType = 'Post'
                        for(i=0;i<jsonData.header.length;i++){
                            var key = jsonData.header[i].KEY;
                            console.log("key:::"+key)
                            var value = jsonData.header[i].VALUE;
                            console.log("value:::"+value)
                            ContentType = key == 'Content-Type'?value:ContentType
                            console.log(ContentType)
                        }

//                        if(jsonData.request == request){
...
//                          parent.layer.alert("请求成功",{icon: 6});
                             $('#responseMsg').val('bbb');
                          },
                          error:function(){
