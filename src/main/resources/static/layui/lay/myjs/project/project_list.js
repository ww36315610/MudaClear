  ...
  csvo = table.render({
    ...
    ,cols: [
     [
      ...
      ,{field:'message', title: '项目说明'}
      ,{field: 'process', title: '进度', width: 150 ,
              templet:function(d){
//              console.log(d);
                  var html = '<div class="layui-progress">';
//                      html += '<div class="layui-progress-bar" lay-percent="' + d.progress +'%"></div>';
                      html += '<div class="layui-progress-bar" lay-percent="' + 50 +'%"></div>';
                      html += '</div>';
                  return html;
              }
       }
...
//      layer.alert('编辑行：<br>'+ JSON.stringify(data))
        openWin(data,'修改信息','/muda/project/project_edit.html',650,600);
    }
  });
...
//    	            body.find("select[name='caseCode']").val(editData.caseCode);
...
//                        body.find("input[name='status']").attr('checked', 'false');
                    ...
                    body.find("input[name='mockAdress']").val(editData.mockAdress);
